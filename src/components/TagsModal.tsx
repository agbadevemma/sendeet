...
const TagsModal = ({ setIsOpen, isOpen, setHighlightedItems, selectedIndex }: Props) => {
  ...
  const handleCreate = async () => {
    ...
    }).unwrap().then((payload) => console.log('fulfilled', payload))
      .catch((error) => console.log('rejected', error)).finally(() => {
        setIsOpen(false)
      });
  }
  return (
          ...
          >
            ...
            <div className="w-full  h-14 p-[6px] flex gap-2 bg-[#F9FAFB] rounded-lg  border border-[#F2F4F7]">
              <div
                ...
                className={`w-full flex items-center justify-center ${!tabs
