...
const HeatmapChart: React.FC = () => {
  ...
  const getColorFromIntensity = (intensity: number): string => {
    const colorCodes: Array<string> = [
      "#E6F7FE", "#B0E5FD", "#54C6FA", "#00AAF7", "#0099DE", "#0088C6",
      "#006694", "#004C6F", "#003B56", "#002A42"
    ];
  ...
  return (
    <div className="w-full flex h-full gap-4">
      <div className="flex flex-col">
        {days.map((day) => (
          <div key={day} className="h-[40px] flex items-center justify-center text-[#667085] text-xs">
            {day}
          </div>
        ))}
      </div>
            ...
            <React.Fragment key={day}>
              {hours.map((hour) => {
                ...
                return (
                  <div
                    key={`${day}-${hour}`}
                    title={`${dataPoint?.count || 0} activities on ${day} at ${hour}`}
